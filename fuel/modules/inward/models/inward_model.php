...
<tr><td></td></tr>
		...
		function saveinwardentry($pid,$pname, $date3,$lno,$icno,$date4,$coil,$fWidth,
							$fThickness,$fLength,$fQuantity,$status,$hno,$pna,$ppartyId,$parentBundleNumber,$grade,$cast,$date5,$jid,$ssid)
	{
		$updateSql = '';
		if(!empty($ppartyId)) {
			$ppartyId = $ppartyId;
		...
		sendSMS('9379394798','Received Coil No '.$pid.'%n'.$coil.' '.$fThickness.'mm x '.$fWidth.'mm '.$fQuantity.'kgs%nOn '.date('d/m/Y').'%nVehicle no '.$lno.'%nRef:'.$icno);
	}
