...
class customer_outward_model extends Base_module_model {
    ...
    function chk_user() {
        ...
        $query = $this->db->select('nPartyName')->from('aspen_tblpartydetails')->where('nPartyName', $userdata['user_name'])->join('fuel_users ', 'aspen_tblpartydetails.nPartyName = fuel_users.user_name', 'left')->get();
